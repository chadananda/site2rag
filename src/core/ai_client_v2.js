...
export class AISessionV2 {
  ...
  setSystemContext(context) {
    ...
    this.lastUsed = Date.now();
    debugLogger.ai(`Session ${this.sessionId} - System context set: ${context.length} chars (~${this.contextTokens} tokens)`);
  }
  /**
  ...
  async call(userPrompt, schema) {
    ...
    debugLogger.ai(`Session ${this.sessionId} - Total new tokens sent: ${userTokens} (saved ${this.contextTokens} tokens)`);
