...
export class MetadataExtractor {
  ...
  extractBreadcrumbs($) {
    ...
    for (const selector of selectors) {
      const breadcrumbs = $(selector)
        .map((_, el) => ({text: $(el).text().trim(), url: $(el).attr('href')}))
        .get();
  ...
  extractLastModified($) {
    return $('meta[name="last-modified"]').attr('content') || '';
  }
  extractAuthor($) {
...
export class NavigationRemover {
  removeAllNavigation(html) {
    ...
    const navigationSelectors = [
      ...
      '.site-header',
      '.site-footer',
      '.page-header:not(.content-header)',
      '.skip-links',
      '.accessibility-nav',
      'script',
      'style',
  ...
  removeTemplateElements($) {
    $('*').each((_, el) => {
      ...
      if (this.isTemplateElement(classes, id)) {
        $el.remove();
        return;
      }
      if (text.length < 100 && this.isTemplateText(text)) {
        $el.remove();
      }
    });
  }
  isTemplateElement(classes, id) {
    const pattern =
      /banner|popup|modal|overlay|widget|component|advertisement|promo|social|share|cookie|gdpr|newsletter|signup/;
...
export class ContentBlockAnalyzer {
  ...
  async analyzeContentBlocks(cleanHtml) {
    ...
    if (ambiguous.length > 0 && aiAvailable) {
      ...
      finalBlocks = finalBlocks.concat(ambiguous.filter((_, i) => !aiRemoveIdxs.includes(i)));
    } else {
...
export class MetadataOnlyExtractor {
  ...
  extractMetadata(html, url) {
    return this.metadataExtractor.extractPageContext(html, url);
  }
}
