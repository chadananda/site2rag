...
def _otsu(t):
    ...
    # histc is faster than scatter_add_ on ROCm for large tensors.
    hist    = _torch.histc(t.float().clamp(0, 255), bins=256, min=0, max=255)
    total   = hist.sum()
    cum_n   = hist.cumsum(0)
...
def _cpu_cv2_variants(img, issues):
    ...
    # Skip NLM for large images — O(n²) cost is prohibitive (30s+) on crops > 200K pixels.
    # Use bilateral filter instead (fast, good enough for OCR denoising).
    h_px, w_px = gray_cv.shape
    skip_nlm = (h_px * w_px) > 200_000
...
def process_image(in_path, out_path, force=False, issues=(), method=None,
    ...
    # Skip slow variant families when a method filter is specified that doesn't need them.
    # NLM/CLAHE/bilateral: CPU-only, ~10-30s each — skip when not needed.
    # CLI (unpaper/imagemagick): subprocess startup ~2-5s each — skip when not needed.
    # This makes layout-detection calls ~30x faster (1s vs 30s per call).
    _CV2_PREFIXES = ('nlm', 'clahe', 'bilateral', 'bleed')
    _CLI_PREFIXES = ('unpaper', 'magick', 'wavelet')
    needs_cv2 = not method or any(p in method for p in _CV2_PREFIXES)
    needs_cli = not method or any(p in method for p in _CLI_PREFIXES)

    if _has_gpu():
        with ThreadPoolExecutor(max_workers=2) as pool:
            cpu_cv2_fut  = pool.submit(_cpu_cv2_variants, img, issues) if needs_cv2 else None
            cli_fut      = pool.submit(_cli_variants,     img, issues) if needs_cli else None
            gpu_vars     = _gpu_variants(img, issues)
            cpu_cv2_vars = cpu_cv2_fut.result() if cpu_cv2_fut else []
            cli_vars     = cli_fut.result() if cli_fut else []
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            cv2_fut  = pool.submit(_cpu_cv2_variants, img, issues) if needs_cv2 else None
            cli_fut  = pool.submit(_cli_variants,     img, issues) if needs_cli else None
            pil_vars = _pil_variants_cpu(img, issues)
            cv2_vars = cv2_fut.result() if cv2_fut else []
            cli_vars = cli_fut.result() if cli_fut else []
