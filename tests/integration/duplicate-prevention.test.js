...
vi.mock('../../src/core/context_processor_simple.js', () => ({
  processDocumentsSimple: vi.fn().mockImplementation(async (docs, aiConfig, progressCallback) => {
    ...
    for (const doc of docs) {
      ...
      results[doc.docId] = blockTexts.map(text => 
        `${text} [[processed by ${aiConfig.processorType} at ${Date.now()}]]`
      );
    }
...
This is the fourth and final paragraph.`;
    ...
    const processor = createParallelAIProcessor(db, {
      provider: 'mock',
      model: 'test',
...
Paragraph 3 of document ${i} final content.`;
    ...
    // Start parallel processor
    const parallelProcessor = createParallelAIProcessor(db, {
      provider: 'mock',
      model: 'test',
      processorType: 'parallel'
    }, 50);
  ...
  it('should show processing distribution between processors', async () => {
    ...
    const processor1 = createParallelAIProcessor(db, {
      provider: 'mock',
      model: 'test',
      processorType: 'parallel1'
    }, 50);
    
    const processor2 = createParallelAIProcessor(db, {
      provider: 'mock',
      model: 'test',
