...
describe('Database Coordination for AI Processing', () => {
  ...
  describe('Stuck Page Recovery', () => {
    it('should reset stuck processing pages', () => {
      ...
      db.db.prepare('UPDATE pages SET last_context_attempt = ? WHERE url = ?')
        .run(oldTime, pages[0].url);
  ...
  describe('Concurrent Processing Simulation', () => {
    ...
    it('should handle mixed success and failure scenarios', () => {
      ...
      const statuses = db.db.prepare('SELECT content_status, COUNT(*) as count FROM pages GROUP BY content_status').all();
  ...
  describe('Batch Processing', () => {
    it('should support claiming pages in batches', () => {
      ...
      const processingCount = db.db.prepare('SELECT COUNT(*) as count FROM pages WHERE content_status = ?').get('processing');
