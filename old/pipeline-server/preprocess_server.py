...
def _warm():
    global _ready
    # Warm up GPU kernels in a background thread so the server starts immediately.
    # The first real request triggers kernel JIT compile on ROCm (~8s); this moves
    # that cost to startup so subsequent requests are fast.
    def _do_warm():
        global _ready
        print('[preprocess-server] warming GPU kernels (background JIT compile)...', flush=True)
        try:
            import numpy as np, tempfile
            from PIL import Image as _PIL
            # 300×300 gradient — enough to trigger all kernel compilations
            arr = np.tile(np.linspace(0, 255, 300, dtype=np.uint8), (300, 1))
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as f:
                _PIL.fromarray(arr).convert('RGB').save(f.name)
                warmup_path = f.name
            out_fd, out_path = tempfile.mkstemp(suffix='.png')
            os.close(out_fd)
            try:
                process_image(warmup_path, out_path, force=True)  # compile all GPU kernels
            finally:
                try: os.unlink(warmup_path)
                except: pass
                try: os.unlink(out_path)
                except: pass
        except Exception as e:
            print(f'[preprocess-server] warmup warning: {e}', flush=True)
        with _startup_lock:
            _ready = True
        print('[preprocess-server] GPU kernels ready', flush=True)

    t = threading.Thread(target=_do_warm, daemon=True)
    t.start()
    # Server starts immediately; requests can be handled (kernels compile on first real request
    # if warmup hasn't finished yet, but that's better than blocking startup entirely)
    _ready = True  # allow requests immediately; warmup is just for speed, not correctness
